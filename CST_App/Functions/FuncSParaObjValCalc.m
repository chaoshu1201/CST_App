...
function ObjectiveVal = FuncSParaObjValCalc(ExportFolder, FileName, GoalVal, RangeVec, Weight)

    % Load exported file of S-parameters
    FullSParaFile = fullfile(ExportFolder, FileName);
    spara_data = load(FullSParaFile);
    ...
    ObjectiveVal = sum(S_Mag_dB(Index) - GoalVal);
